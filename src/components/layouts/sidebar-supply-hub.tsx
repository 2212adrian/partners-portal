...
const tabConfig: Record<OrderTab, { label: string; icon: typeof Clock; color: string; bgColor: string; countKey: keyof OrderCounts }> = {
  "pending": { 
    label: "Pending", 
    icon: Clock, 
...
const MainNavItem = ({
  ...
  <button
    ...
    className={`relative flex items-center justify-between w-full px-4 py-3 transition-all group ${
      active ? "bg-emerald-50 border-r-4 border-emerald-500" : "hover:bg-slate-50"
    }`}
  >
    <div className="flex items-center gap-3">
      <div className={`p-1.5 rounded-lg ${active ? "bg-emerald-100" : "bg-slate-100"} transition-colors`}>
...
const StatusTabItem = ({
  ...
  >
    <div className="flex items-center gap-3">
      <div className={`p-1 rounded-lg ${active ? bgColor : "bg-slate-100"} transition-colors`}>
        <Icon size={12} className={active ? color : "text-slate-400"} />
      </div>
      <span
        className={`text-[11px] font-medium transition-colors ${
          active ? "text-slate-700" : "text-slate-500 group-hover:text-slate-600"
        }`}
      >
        {label}
      </span>
...
export const SidebarSupplyHub: React.FC<SidebarSupplyHubProps> = ({
  ...
  const [myOrdersExpanded, setMyOrdersExpanded] = React.useState(activeMainSection === "my-orders");
  const tabs = Object.entries(tabConfig) as [OrderTab, typeof tabConfig[OrderTab]][];
  ...
  const handleMyOrdersClick = () => {
    // If clicking on My Orders when it's already expanded and active, just toggle
    if (activeMainSection === "my-orders" && myOrdersExpanded) {
      setMyOrdersExpanded(!myOrdersExpanded);
    } else {
      // Otherwise, navigate to My Orders and expand
      onMyOrdersClick();
      setMyOrdersExpanded(true);
    }
  };
  ...
  return (
    <aside className="w-72 h-full bg-white border-r border-slate-200 flex flex-col overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-100">
        ...
        >
          ...
          <span className="text-[12px] font-bold">Back to Main Menu</span>
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium">Marketplace</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-100 bg-slate-50/50">
        ...
        <button className="w-full text-left text-[12px] font-medium text-slate-600 hover:text-emerald-600 transition-colors">
          Manual Inventory →
        </button>
      </div>
    </aside>
